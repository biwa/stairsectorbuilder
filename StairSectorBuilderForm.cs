...
namespace CodeImp.DoomBuilder.StairSectorBuilderMode
{
	public partial class StairSectorBuilderForm : DelayedForm
	{
		...
		public uint NumberOfSectors
		{
			...
			get { return (uint)numberofsectors.GetResult(0); }
		...
		public CheckBox RemoveRedundantVertices
		{
			get { return removeredundantvertices; }
			set { removeredundantvertices = value; }
		}
		...
		{
			get { return lightingmodification.Checked; }
			set { lightingmodification.Checked = value; }
		}

		public int LightingModification
		{
			get { return lightingmod.GetResult(0); }
			set { lightingmod.Text = value.ToString(); }
		}
		...
		public int LightingBase
		{
			set { lightingbase.Text = value.ToString(); }
			get { return lightingbase.GetResult(0); }
		}
		...
		private void ComputeHeights()
		{
			...
			{
				lightingfirst.Text = (Int32.Parse(lightingbase.Text)).ToString();
				lightinglast.Text = (Int32.Parse(lightingbase.Text) + Int32.Parse(lightingmod.Text) * ((Int32.Parse(numberofsectors.Text) - 1) * stepmultiplier)).ToString();
			}
		}
		...
		{
			if(BuilderPlug.Me.Prefabs.Count > 0 && BuilderPlug.Me.Prefabs[0].name == "[Default]")
				SavePrefab("[Previous]", true, 1);
			else
		...
		private void numberofsectors_WhenTextChanged(object sender, EventArgs e)
		{
			...
			if (numberofsectors.Text != "" && numberofsectors.GetResult(1) != 0)
			{
				ComputeHeights();
				btnOK.Enabled = true;
			}
		...
		private void StairSectorBuilderForm_Load(object sender, EventArgs e)
		{
			...
			// try to load the default prefab if it exists
			LoadPrefab("[Default]");

			// reset the prefab name
			prefabname.Text = "";
		...
		private void StairSectorBuilderForm_Shown(object sender, EventArgs e)
		{
			...
			if (General.Map.Map.SelectedSectorsCount > 0)
			{
				...
				removeredundantvertices.Checked = false;
				removeredundantvertices.Enabled = true;
			}
		...
		private void singlesectors_CheckedChanged(object sender, EventArgs e)
		{
			...
			{
				singledirection.Enabled = false;
				removeredundantvertices.Enabled = false;
			}
		...
		private void prefabload_Click(object sender, EventArgs e)
        {
			...
			LoadPrefab(BuilderPlug.Me.Prefabs[prefabs.SelectedIndices[0]].name);
		}
		...
		private void prefsetdefault_Click(object sender, EventArgs e)
		{
			SavePrefab("[Default]", false, 0);
		}

		private void removeredundantvertices_CheckedChanged(object sender, EventArgs e)
		{
			DoRedrawDisplay();
		}

		private void lightingmod_WhenTextChanged(object sender, EventArgs e)
		{
			if (lightingmod.GetResult(0) == 0) lightingmod.Text = "0";
			if (lightingbase.GetResult(0) == 0) lightingbase.Text = "0";
			ComputeHeights();
		}

		private void lightingbase_WhenTextChanged(object sender, EventArgs e)
		{
			if (lightingbase.GetResult(0) == 0) lightingbase.Text = "0";
			ComputeHeights();
		}
